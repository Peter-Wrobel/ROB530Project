function varargout = run_fused(stepsOrData, pauseLen, makeVideo, filter_name)
...
for t = 1:numSteps
    ...
    %=================================================
    motionCommand = data(t,7:9)'; % [Trans_vel,Angular_vel,gamma]' noisy control command
    observation_1 = data(t,1:3)';   % [bearing, range, landmark_id]' noisy observation
    observation_2 = data(t,4:6)';   % [bearing, range, landmark_id]' noisy observation
    observation = [observation_1 observation_2];
    
    Y = data(t,22:24); % landmark 1 position relative to the robot
    Y2 = data(t, 25:27);% landmark 2 position relative to the robot
%     landmark_ids = [data(t,3), data(t,22)];
    ...
    plot([initialStateMean(1) data(1,19)], [initialStateMean(2) data(1,20)], 'Color', NOISEFREE_PATH_COL);

    % draw actual path (i.e., ground truth)
    plot(data(1:t,16), data(1:t,17), 'Color', ACTUAL_PATH_COL, 'linewidth', 2);
    plotrobot(x, y, theta, 'black', 1, 'cyan');

    % draw noise free motion command path
    plot(data(1:t,19), data(1:t,20), 'Color', NOISEFREE_PATH_COL, 'linewidth', 2);
    plot(data(t,19), data(t,20), '*', 'Color', NOISEFREE_PATH_COL, 'linewidth', 2);
    ...
    plot([x x+cos(theta+observation(4))*100], [y y+sin(theta+observation(4))*100], 'Color', OBSERVED_BEARING_COLOR, 'linewidth', 2);
